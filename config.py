...
class SchedulerConfig(object):
    ...
    batch_increments = os.environ.get('BATCH_INCREMENTS') or 5  # How many Defined targets to add in each iteration. Defined target can resolve into many Scan targets when multiple IPs are ressolved by DNS.
    batch_size = os.environ.get('BATCH_SIZE') or 5  # Desired batch size of Scan targets per each scan batch. The actual upper limit will be: batch_size - batch_increments + (batch_increments * max_records_per_resolve)
